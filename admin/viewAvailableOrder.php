        ...
        ?>
                ...
                <div class="modal-content">
                        ...
                        <?php
                         ...
                         $con = mysqli_connect("127.0.0.1","root-1","nibh@1713","nibh-v1");
                            if(isset($_POST['editEnquiry'])){
                                ...
                                $sql = "SELECT availableorder.id,availableorder.OrderId,availableorder.OrderDate,availableorder.OrderYear,availableorder.MobileNo,availableorder.CustomerName,stream.streamName,branch.branchName,year.yearName,sem.semName,books.bookName,publication.publicationName,author.authorName,availableorder.Quantity,availableorder.Delivery,availableorder.Address,availableorder.Instructions,availableorder.OrderPlacedBy,availableorder.Location FROM (((((((availableorder INNER JOIN stream ON availableorder.Stream = stream.streamID) INNER JOIN branch ON availableorder.Branch = branch.branchID)INNER JOIN year ON availableorder.Year = year.yearID)INNER JOIN sem ON availableorder.Sem = sem.semID)INNER JOIN books ON availableorder.Book = books.bookID)INNER JOIN publication ON availableorder.Publication = publication.publicationID)INNER JOIN author ON availableorder.Author = author.authorID) where id = '$id' ";
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger" onclick="window.location.href = 'http://newindiabookhouse.com/availableOrder.php';">Close</button>
                    </div>
