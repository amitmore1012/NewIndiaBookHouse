        ...
        ?>
                ...
                <div class="modal-content">
                        ...
                        <?php
                         ...
                         $con = mysqli_connect("127.0.0.1","root-1","nibh@1713","nibh-v1");
                            if(isset($_POST['editEnquiry'])){
                                ...
                                $sql = "SELECT completedorder.id, completedorder.OrderId, completedorder.OrderDate, completedorder.OrderYear, completedorder.MobileNo, completedorder.CustomerName, stream.streamName ,branch.branchName, year.yearName, sem.semName, books.bookName, publication.publicationName, author.authorName, completedorder.Quantity, completedorder.Delivery, completedorder.Instructions, completedorder.Address,completedorder.OrderPlacedBy, completedorder.Location FROM (((((((completedorder INNER JOIN stream ON completedorder.Stream = stream.streamID) INNER JOIN branch ON completedorder.Branch = branch.branchID)INNER JOIN year ON completedorder.Year = year.yearID) INNER JOIN sem ON completedorder.Sem = sem.semID) INNER JOIN books ON completedorder.book = books.bookID) INNER JOIN publication ON completedorder.Publication = publication.publicationID) INNER JOIN author ON completedorder.Author = author.authorID) where id = '$id' ";
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger" onclick="window.location.href = 'http://newindiabookhouse.com/completedOrder.php';">Close</button>
                    </div>
